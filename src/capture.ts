...
export class VideoCapture extends CaptureBase {
  ...
  async initialize() {
    ...
    // await this.page!.addScriptTag({ src: "https://bundle.run/buffer@6.0.1" })

    await this.page!.addScriptTag({ content: readFileSync(join(__dirname, 'assets', 'buffer-6.0.1.min.js')).toString() })
